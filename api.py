...
"""
import copy, tools, blockchain, custom, random, transactions, sys, time, networking, forth
def sign(tx, privkey):
...
def easy_add_transaction(tx_orig, DB={}, privkey='default'):
    ...
    if 'recent_hash' not in tx and length>0:
        rh=tools.db_get(max(1, length-2))['block_hash']
        tools.log('recent hash: ' +str(rh))
        tx['recent_hash']=rh
...
def help_(DB, args):      
    tell_about_command={
        ...
        'make_contract':'./cli.py make_contract contrct_id 500 : square dup * ; : pythag square swap square + ;',
        'contract_do':'./cli contract_do contract_id 10000 3 4 pythag',
...
def buy_block(DB, args):
    ...
    return block
def make_contract(DB, args):
    if len(args)<3:
        return('not enough inputs')
    tx={'type':'make_contract'}
    tx['id']=args[0]
    tx['amount']=int(args[1])
    tools.log('msg: ' +str(args[2]))
    d=forth.read_definitions(args[2])
    tools.log('d: ' +str(d))
    tx['mem']=d
    if type(tx['mem']) in [str, unicode]:
        return('bad forth script: ' +str(tx['mem']))
    return easy_add_transaction(tx)
def contract_do(DB, args):
    tools.log('args: ' +str(args))
    if len(args)<3:
        return('not enough inputs')
    tx={'type':'contract_do'}
    tx['contract_id']=args[0]
    tx['amount']=int(args[1])
    tx['code']=args[2]
    contract=tools.db_get(tx['contract_id'])
    if 'mem' not in contract:
        return('contract does not exist yet')
    tx['old_mem']=contract['mem']
    tools.log('contract do: ' +str(tx))
    return easy_add_transaction(tx)
def pass_(DB, args): return ' '
def error_(DB, args): return error
Do={'spend':spend, 'help':help_, 'blockcount':blockcount, 'txs':txs, 'balance':balance, 'my_balance':my_balance, 'b':my_balance, 'info':info, 'patty_info':patty_info, '':pass_, 'DB':DB_print, 'my_address':my_address, 'log':log, 'stop':stop_, 'commands':commands, 'pushtx':pushtx, 'peers':peers, 'buy_block':buy_block, 'make_contract':make_contract, 'contract_do':contract_do}
